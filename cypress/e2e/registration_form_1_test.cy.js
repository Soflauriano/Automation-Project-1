...
Assignment 2:
 ...
 2. Replace text ‘123pass’ in the first test with your own chosen password (2 places) - passwords should match
...
describe('This is first test suite,Soraia Lauriano', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Aurora')
        cy.get('#lastName').type('Cinderella')
        cy.get('input[name="password"]').type('123pass')
        cy.get('[name="confirm"]').type('123pass')
        cy.get('#username').type('Something')
        //This is my first comment, todays is 24may03
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('input[name="password"]').type('123pass')
        cy.get('[name="confirm"]').type('123pass123')
